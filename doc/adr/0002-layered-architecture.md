...
validates them against the YaCy models in `yacywire`, and hands the validated models to the
...
the shared `yacywire` models. `cmd` is the only place they are wired together.
...
Validation is defined once in `yacywire` and runs at the boundaries that admit external data.
