...
Three services read every scrape request. `yacycrawler` follows the links,
`corpustext` keeps the text, and the peer stores the words. Each one fetches the
page for itself, so the browser renders it three times and the origin sees three
visits.

`pagecache` is a Squid proxy in front of `renderproxy`. The first request
renders the page; the others read what is stored. When the three arrive
together, one goes on and the other two wait for it.

## The change

The three services point `SCRAPE_PROXY_URL` at the cache:

```yaml
      SCRAPE_PROXY_URL: http://pagecache:3128
```

How long a page stays good is the origin's decision. `renderproxy` passes on the
`Cache-Control` and `Last-Modified` it received, and Squid obeys them.
...
docker compose logs -f pagecache
```
