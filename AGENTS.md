...
Naming — derivation: a function that returns a value is named for the value, joined to its source by the preposition that names the relation: `Of` an attribute the subject has, `From` a value derived from it, `For` a value serving it (`replicas.HoldersOf(tx, posting)`, `interval.widenedFrom(previousInterval)`). A function that acts keeps its verb.

Naming — a hard name means a weak unit: if a good name will not come, or the user calls a name unreadable, it is a design problem, not a wording problem. Look at what the code does and owns. A name that hides a side effect, or names the returned value while the body also reserves, starts, or retries, is a job no unit owns yet. Give that job its own unit, then name it.
