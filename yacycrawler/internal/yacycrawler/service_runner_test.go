...
func TestRunServiceDrivesOrdersToIngest(t *testing.T) {
	...
	go func() { runDone <- yacycrawler.RunService(ctx, cfg, source) }()
...
func serviceConfig(natsURL string) yacycrawler.ServiceConfig {
	getenv := func(key string) string {
		switch key {
		case yacycrawler.EnvNATSURL:
			return natsURL
		case yacycrawler.EnvWorkers:
			return "1"
		default:
			return ""
		}
	}
	cfg, err := yacycrawler.LoadServiceConfig(getenv)
	if err != nil {
		panic(err)
	}
	return cfg
