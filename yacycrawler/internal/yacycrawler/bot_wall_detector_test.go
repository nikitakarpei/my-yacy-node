...
func TestBotWallDetector(t *testing.T) {
	...
	detector := yacycrawler.NewBotWallDetector()
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			page := yacycrawler.FetchedPage{Body: []byte(tc.body)}
...
func TestBotWallDetectorScansBoundedPrefix(t *testing.T) {
	...
	page := yacycrawler.FetchedPage{Body: []byte(body)}
	if yacycrawler.NewBotWallDetector().IsBotWall(page) {
