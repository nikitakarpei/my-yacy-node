...
func (r recordingIndexer) Index(context.Context, yacycrawlcontract.ExtractedText) error {
	if r.fail {
...
func TestArtifactConsumerAcksOnSuccessfulIndex(t *testing.T) {
	acked := make(chan string, 1)
	data, err := yacycrawlcontract.MarshalExtractedText(yacycrawlcontract.ExtractedText{CanonicalURL: "https://example.com/"})
	if err != nil {
		t.Fatalf("marshal: %v", err)
	}
	source := fakeSource{iterator: &fakeIterator{messages: []jetstream.Msg{newFakeMsg(data, acked)}}}
	consumer := textsubscription.NewArtifactConsumer(source, recordingIndexer{}, 1)
...
func TestArtifactConsumerNaksOnIndexFailure(t *testing.T) {
	acked := make(chan string, 1)
	data, err := yacycrawlcontract.MarshalExtractedText(yacycrawlcontract.ExtractedText{CanonicalURL: "https://example.com/"})
	if err != nil {
		t.Fatalf("marshal: %v", err)
	...
	source := fakeSource{iterator: &fakeIterator{messages: []jetstream.Msg{newFakeMsg(data, acked)}}}
	consumer := textsubscription.NewArtifactConsumer(source, recordingIndexer{fail: true}, 1)
...
func TestArtifactConsumerTermsOnDecodeFailure(t *testing.T) {
	acked := make(chan string, 1)
	source := fakeSource{iterator: &fakeIterator{messages: []jetstream.Msg{newFakeMsg([]byte("not json"), acked)}}}
	consumer := textsubscription.NewArtifactConsumer(source, recordingIndexer{}, 1)
