...
| `YACY_DISTRIBUTION_PARTITION_EXPONENT` | `4` | Ring partition exponent; must match the network's `network.unit.dht.partitionExponent`. A change makes the recorded ring occupancy wrong, and the operator recreates the store. |
