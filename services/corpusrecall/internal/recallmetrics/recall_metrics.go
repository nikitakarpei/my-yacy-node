...
func New() *RecallMetrics {
	...
	metrics := &RecallMetrics{
		...
		unavailable: prometheus.NewCounterVec(prometheus.CounterOpts{
			...
			Help: "Representations reported unavailable by the deadline, by kind.",
		}, []string{"kind"}),
	}
...
func (m *RecallMetrics) RepresentationRecalled(kind pagerecall.Kind) {
	m.recalled.WithLabelValues(string(kind)).Inc()
}

func (m *RecallMetrics) RepresentationUnavailable(kind pagerecall.Kind) {
	m.unavailable.WithLabelValues(string(kind)).Inc()
}

func (m *RecallMetrics) Handler() http.Handler {
