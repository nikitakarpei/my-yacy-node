...
func TestPlacingAnOrderFailsWhenTheSeedURLCannotBeCanonicalized(t *testing.T) {
	placement := crawlorderplacersjetstream.NewCrawlOrderPlacement(ordersStream(t), ordersSubject)

	if err := placement.Place(context.Background(), "://nonsense"); err == nil {
		t.Fatal("expected an error for a url that cannot be canonicalized")
	}
}
