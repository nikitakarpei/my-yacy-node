...
var scrapeProfile = yacycrawlcontract.CrawlProfile{
	Name:            "firecrawlshim",
...
func NewCrawlOrderPlacement(orders jetstream.JetStream, subject string) *CrawlOrderPlacement {
...
func (p *CrawlOrderPlacement) Place(ctx context.Context, canonicalURL string) error {
	seedURL, err := canonicalurl.CanonicalURLOf(canonicalURL)
	if err != nil {
		return fmt.Errorf("canonicalize seed url %q: %w", canonicalURL, err)
	}
