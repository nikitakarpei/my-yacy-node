...
const bucketName = "query-word-amounts"
...
func (r *recordedFailures) AmountLookupFailed(_ context.Context, word yacymodel.Hash, _ error) {
	r.lookups = append(r.lookups, word)
}

func (r *recordedFailures) AmountStoreFailed(_ context.Context, word yacymodel.Hash, _ error) {
...
func TestAnAmountOneInstanceRemembersIsReadBackByAnother(t *testing.T) {
	...
	observers := querywordamountsjetstream.QueryWordAmountsObservers{failures}
	berlin, weather := yacymodel.WordHash("berlin"), yacymodel.WordHash("weather")
	querywordamountsjetstream.New(server.bucket, observers).
		Remember(t.Context(), map[yacymodel.Hash]int{berlin: 12})

	got := querywordamountsjetstream.New(server.bucket, observers).
		AmountsOf(t.Context(), []yacymodel.Hash{berlin, weather})
...
func TestALookupTheBucketRefusesIsReportedAndIsAMiss(t *testing.T) {
	...
	server := bucketOnATestServerFor(t)
	failures := &recordedFailures{}
	amounts := querywordamountsjetstream.New(
		server.bucket, querywordamountsjetstream.QueryWordAmountsObservers{failures},
	)
	berlin := yacymodel.WordHash("berlin")
	amounts.Remember(t.Context(), map[yacymodel.Hash]int{berlin: 12})
	server.deleteTheBucket(t)

	got := amounts.AmountsOf(t.Context(), []yacymodel.Hash{berlin})

	if len(got) != 0 || len(failures.lookups) != 1 || failures.lookups[0] != berlin {
		t.Fatalf("AmountsOf = %v with lookup failures %v, want a miss reported for berlin",
...
func TestAValueThatIsNotAnAmountIsReportedAndIsAMiss(t *testing.T) {
	...
	got := querywordamountsjetstream.New(
		server.bucket, querywordamountsjetstream.QueryWordAmountsObservers{failures},
	).AmountsOf(t.Context(), []yacymodel.Hash{berlin})

	if len(got) != 0 || len(failures.lookups) != 1 {
		t.Fatalf("AmountsOf = %v with lookup failures %v, want a reported miss",
			got, failures.lookups)
	}
}
