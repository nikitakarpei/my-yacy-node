...
type peerNetwork struct {
	...
	discoveryAsksPut                                []peerasks.SearchDocumentsAsk
...
func (n *peerNetwork) AskForSearchDocuments(
	...
	if len(asks) > 0 && len(asks[0].DocumentsToMatch) > 0 {
		return n.crossCheckAskOutcomes(ctx, asks)
	}
	...
	placesPut := n.placesOfTheDiscoveryAsksPutAmong(asks)
	for place, ask := range asks {
		if _, put := placesPut[place]; !put {
			continue
		}
		n.discoveryAsksPut = append(n.discoveryAsksPut, ask)
...
func (n *peerNetwork) placesOfTheDiscoveryAsksPutAmong(
	asks []peerasks.SearchDocumentsAsk,
) map[int]struct{} {
...
func (n *peerNetwork) crossCheckAskOutcomes(
	ctx context.Context,
	asks []peerasks.SearchDocumentsAsk,
) peerasks.SearchDocumentsAskOutcomes {
...
func TestAPeerThatClaimsAnotherVersionIsAskedAgain(t *testing.T) {
	...
	if amount := amountOfCrossChecksPutTo("third", network.crossCheckAsks); amount != 2 {
		t.Fatalf(
...
func TestAReplicaTheFirstRoundLeftUnaskedIsAskedAboutTheDocumentsItsPartitionLeftOut(
	...
	if got := addressesAskedToCrossCheck(network.crossCheckAsks); !slices.Equal(
		got, []string{"third"},
	) {
		t.Fatalf(
...
func TestEachPeerGetsAtMostOneSearchCallPerQuery(t *testing.T) {
	...
	for _, ask := range network.discoveryAsksPut {
...
func TestIgnoringPeersAreAskedFirstAndHonoringPeersLastInTheFirstRound(t *testing.T) {
	...
	if got := replicasAskedForTheWordInOrder(
		yacymodel.WordHash(secondWord), network.discoveryAsks,
...
func TestAPeerWhoseAbstractHeldADocumentOutsideTheDocumentsToMatchIsNotAskedToCrossCheckByTheNextQuery(
	...
	if amount := amountOfCrossChecksPutTo("third", network.crossCheckAsks); amount != 1 {
		t.Fatalf(
...
func TestAReplicaOfTwoQueryWordsWithPartialAbstractsIsAskedToCrossCheckOnce(t *testing.T) {
	...
	if amount := observer.performed[0].CrossCheckRound.
...
func TestEveryReplicaTheFirstRoundLeftUnaskedIsAskedTheSameDocuments(t *testing.T) {
	...
	network := networkOf(map[string]map[string][]string{
		...
		"fourth": {secondWord: documentsOfTheLeadingQueryWord},
	})
	network.documentsPerAnswerOfEachPeer = map[string]int{"second": 1}
	network.replicasPutPerWord = map[string]int{secondWord: 1}
	...
	wanted := documentsInTheirHashOrder(
		documentHashesOf(documentsOfTheLeadingQueryWord[1:]),
	)
	for _, ask := range network.crossCheckAsks {
...
func TestTheCandidatesOverTheCeilingAreCountedAsNoPeerTook(
	...
	if len(network.crossCheckAsks) != 2 {
		t.Fatalf(
			"the spread put %v, want a cross-check ask to each replica left unasked",
			network.crossCheckAsks,
		)
	}
...
func TestADocumentSentToCrossCheckForTwoQueryWordsIsCountedForEach(t *testing.T) {
	...
	network := networkOf(map[string]map[string][]string{
		...
		"fifth":  {thirdWord: documentsOfTheLeadingQueryWord},
	})
...
func TestOnlyThePeersResponsibleForAWordAreAskedWhatTheyHoldForIt(t *testing.T) {
	...
	if len(network.discoveryAsks) != 2 {
		t.Fatalf(
			"%d abstracts and cross-check asks were put, want one for each responsible peer",
...
func TestACandidateOfOnePartitionIsAskedOnlyOfTheReplicasInThatPartition(t *testing.T) {
	...
	if !slices.Equal(got, wanted) || network.crossCheckAsks[0].Partition != 1 {
