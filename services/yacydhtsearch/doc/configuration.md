...
Without a NATS address each instance caches its own rankings, and a restart drops them. With one, the instances answer a repeated query from the same ranking and share the query word amounts. An address that does not answer stops the service from starting.
...
| `YACYDHTSEARCH_QUERY_WORD_AMOUNT_LIFETIME` | `6h` | Time the service remembers how many documents the peers hold for a query word. A query whose words all have a remembered amount takes no sample. |
| `YACYDHTSEARCH_QUERY_WORD_AMOUNTS_CAPACITY` | `100000` | Most query words whose amount the service remembers. |
| `YACYDHTSEARCH_NATS_URL` | in-memory | NATS address that caches rankings and query word amounts for every instance. |
...
| `YACYDHTSEARCH_URL_METADATA_ASK_DOCUMENTS_FLOOR` | `25` | Fewest documents one URL metadata call asks a peer about when the peer answers slowly or fails. A peer that the service asks for fewer documents than `YACYDHTSEARCH_URL_METADATA_ASK_DOCUMENTS_CEILING` because of its answer speed is asked first about the documents that fewer other peers hold. |
| `YACYDHTSEARCH_URL_METADATA_ASK_TARGET_TIME` | `1s` | Answer time that the service plans for one URL metadata call to a peer. The service asks each peer for the documents it answered per second, times this time, between the floor and the ceiling. The service learns this rate from calls that asked the peer for as many documents as its ceiling allowed, from cancelled calls and from failed calls. A failed call lowers the value for the peer. A call that the service cancels lowers it only to what the peer answered until the cancel. |
