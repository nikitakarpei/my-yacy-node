...
type SearchProgressMetrics struct {
	searchesServed prometheus.Counter
	searchFailures prometheus.Counter
}

func New(registry prometheus.Registerer) *SearchProgressMetrics {
	metrics := &SearchProgressMetrics{
...
func (m *SearchProgressMetrics) SearchServed(_ context.Context, _ string, _ int) {
	m.searchesServed.Inc()
}

func (m *SearchProgressMetrics) SearchFailed(_ context.Context, _ string, _ error) {
