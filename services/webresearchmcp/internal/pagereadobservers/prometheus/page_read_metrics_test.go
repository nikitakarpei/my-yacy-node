...
func TestPageReadMetricsCountsEveryFactItIsTold(t *testing.T) {
	ctx := context.Background()
	registry := prometheusclient.NewRegistry()
	metrics := pagereadobserversprometheus.New(registry)
...
func TestPageReadMetricsExposesNoPageAddress(t *testing.T) {
	ctx := context.Background()
	registry := prometheusclient.NewRegistry()
	metrics := pagereadobserversprometheus.New(registry)
	pageURL := canonicalurltest.CanonicalURLOf(t, unexposedPageAddress)
