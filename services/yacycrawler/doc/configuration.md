...
|---|---|---|
| `CRAWL_NATS_URL` | required | NATS server the crawler consumes crawl orders from and reports crawled pages to. |
...
The crawler reports every page it read on the `YACY_CRAWL_PAGES` stream, which it creates
