...
func (r *Recorder) Record(ctx context.Context, requested, canonical string) error {
	if requested == canonical {
		return nil
	}
	key := yacycrawlcontract.RedirectResolutionKey(requested)
	if _, err := r.bucket.Put(ctx, key, []byte(canonical)); err != nil {
