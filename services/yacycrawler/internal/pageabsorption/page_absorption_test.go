...
func absorb(t *testing.T, a *Absorption, outcome crawlcapability.FetchOutcome) []string {
	t.Helper()
...
func TestAbsorbDisposesUnsupportedMediaType(t *testing.T) {
	observer := newObserver()
	a := newAbsorption(fakeExtract{err: crawlcapability.ErrUnsupportedMediaType},
		&recordingResolve{},
...
func TestAbsorbDisposesContainerOverflow(t *testing.T) {
	observer := newObserver()
	a := newAbsorption(fakeExtract{err: crawlcapability.ErrContainerOverflow},
		&recordingResolve{},
		feeds(&fakeFeed{representation: yacycrawlcontract.PageRepresentationKindRWI}), observer)
