package pagereadabletext_test

import (
	"testing"

	"github.com/nikitakarpei/yacy-rwi-node/yacycrawler/internal/crawlcapability"
	"github.com/nikitakarpei/yacy-rwi-node/yacycrawler/internal/pagereadabletext"
)

func TestReadableHTMLDerivationDeclaresReadableHTMLToReadableText(t *testing.T) {
	derivation := pagereadabletext.NewReadableHTMLDerivation()
	if source := derivation.SourceFormat(); source != crawlcapability.PageContentFormatReadableHTML {
		t.Fatalf("source format = %q, want readable-html", source)
	}
	if target := derivation.TargetFormat(); target != crawlcapability.PageContentFormatReadableText {
		t.Fatalf("target format = %q, want readable-text", target)
	}
}

func TestReadableHTMLDerivationFlattensMarkup(t *testing.T) {
	body, err := pagereadabletext.NewReadableHTMLDerivation().Derive(
		"https://example.com/",
		[]byte(`<p>first</p><p>second</p>`),
	)
	if err != nil {
		t.Fatalf("derive: %v", err)
	}
	if string(body) != "first\nsecond" {
		t.Fatalf("markup not flattened: %q", body)
	}
}
