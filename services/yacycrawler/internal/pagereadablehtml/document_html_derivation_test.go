...
func TestDeriveDeclaresDocumentHTMLToReadableHTML(t *testing.T) {
	derivation := pagereadablehtml.NewDocumentHTMLDerivation()
	if source := derivation.SourceFormat(); source != crawlcapability.PageContentFormatDocumentHTML {
		t.Fatalf("source format = %q, want document-html", source)
	}
	if target := derivation.TargetFormat(); target != crawlcapability.PageContentFormatReadableHTML {
		t.Fatalf("target format = %q, want readable-html", target)
	}
}

func TestDeriveExtractsMainArticle(t *testing.T) {
	body, err := pagereadablehtml.NewDocumentHTMLDerivation().
...
func TestDeriveEmptyContentUnextractable(t *testing.T) {
	_, err := pagereadablehtml.NewDocumentHTMLDerivation().
