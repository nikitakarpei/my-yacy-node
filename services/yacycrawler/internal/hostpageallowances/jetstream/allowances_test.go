...
func spendPage(
	...
	t.Helper()
	spent, err := allowances.SpendPage(context.Background(), orderID, url, host, maxPages)
	if err != nil {
...
func TestAVisitSpendsOneHostPageHoweverOftenItIsResumed(t *testing.T) {
	...
	for resumption := range 3 {
		if !spendPage(t, allowances, url, 1) {
	...
	if spendPage(t, allowances, other, 1) {
...
func TestAHostThatAllowsNoPagesSpendsNone(t *testing.T) {
	if spendPage(t, hostPageAllowances(t), pageURL(t), 0) {
...
func TestAnUnlimitedHostAllowanceSpendsNothing(t *testing.T) {
	...
	for range 3 {
		url := pageURL(t)
		if !spendPage(t, allowances, url, yacycrawlcontract.UnlimitedPagesPerHost) {
...
func TestTheSameHostSpendsSeparatelyUnderAnotherOrder(t *testing.T) {
	allowances := hostPageAllowances(t)
	url := pageURL(t)
	spendPage(t, allowances, url, 1)

	other := canonicalurltest.CanonicalURLOf(t, "http://host/other")
	spent, err := allowances.SpendPage(context.Background(), "o2", other, host, 1)
	if err != nil {
		t.Fatalf("spend page: %v", err)
	}
	if !spent {
...
func TestOnlyOneOfTwoConcurrentVisitsSpendsTheLastPageOfAHost(t *testing.T) {
	...
	for page := range 2 {
		...
		go func() {
			defer spending.Done()
			spent, err := allowances.SpendPage(ctx, orderID, url, host, 1)
			if err != nil {
