...
func unextractable(_ []byte) ([]byte, error) {
	return nil, fmt.Errorf("%w: empty", ErrUnderivable)
}

func resolverFor(derivations ...Derivation) *PageFormats {
	return New(derivations).ForPage(
		"http://host/",
		FormatDocumentHTML,
		[]byte("document"),
	)
...
func TestResolvePrefersEarlierCandidateOverFallback(t *testing.T) {
	...
	cached, ready, err := resolver.Resolve(FormatReadableText)
