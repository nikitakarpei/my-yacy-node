...
func ledgerOf(claims *fakeClaims, hostPages *fakeHostPageAllowances) *pagevisitallowance.Ledger {
	return pagevisitallowance.New(claims, hostPages, retrydelay.Bounds{
		Floor:   time.Second,
		Ceiling: time.Minute,
	})
}
...
func TestAHostPageIsGrantedWhenTheHostStillHasPagesToSpend(t *testing.T) {
	...
	allowance, err := ledgerOf(&fakeClaims{}, hostPages).
		HostPageFor(t.Context(), pageVisitOf(t), 5)
	if err != nil {
