...
func fetchedOutcome() pagevisit.FetchOutcome {
	return pagevisit.FetchOutcome{
		Status: pagevisit.FetchSucceeded, FinalURL: "http://host/", ContentType: "text/html",
		Body: []byte("x"),
	}
}
...
func TestVisitAbsorbsFetchedPage(t *testing.T) {
	...
	visitor := pagevisit.NewPageVisit(
		fetch, alwaysdue.AlwaysDue{}, &fakeAbsorption{links: map[string][]string{
...
func TestVisitAbsorptionErrorFails(t *testing.T) {
	...
	visitor := pagevisit.NewPageVisit(
		fetch, alwaysdue.AlwaysDue{}, &fakeAbsorption{err: errors.New("absorb boom")},
		newObserver(), &manualClock{},
	)
...
func TestVisitDisposesNotAPage(t *testing.T) {
	...
	visitor := pagevisit.NewPageVisit(
		fetch, alwaysdue.AlwaysDue{}, &fakeAbsorption{}, observer, &manualClock{},
	)
	...
	if observer.disposed[pagevisit.DisposalFetchFailed] != 1 {
...
func TestVisitCeasesOnHTTPCease(t *testing.T) {
	...
	if observer.refusals[pagevisit.RefusalCease] != 1 {
...
func TestVisitReportsTransient(t *testing.T) {
	fetch := &fakeFetch{outcomes: map[string][]pagevisit.FetchOutcome{
		"http://host/": {{Status: pagevisit.FetchTransient}},
	}}
	visitor := pagevisit.NewPageVisit(
		fetch, alwaysdue.AlwaysDue{}, &fakeAbsorption{}, newObserver(), &manualClock{},
	)
