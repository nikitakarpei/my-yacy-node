...
import (
	...
	"github.com/nikitakarpei/yacy-rwi-node/yacycrawler/internal/pagevisitclaims/jetstream"
)
...
func pageVisitClaims(t *testing.T) *jetstream.Claims {
	t.Helper()
...
func TestTheFirstHolderTakesTheClaimAndTheNextFindsItHeldElsewhere(t *testing.T) {
	claims := pageVisitClaims(t)
	url := pageURL(t)

	if taken := claim(t, claims, url, holder); taken != pagevisitclaim.Taken {
		t.Fatalf("the first holder got %q, want the claim taken", taken)
	}
	if next := claim(t, claims, url, "another holder"); next != pagevisitclaim.HeldElsewhere {
...
func TestTheHolderOfAClaimResumesItsOwnVisit(t *testing.T) {
	...
	if resumed := claim(t, claims, url, holder); resumed != pagevisitclaim.Resumed {
...
func TestTheSameURLIsClaimableAgainUnderAnotherOrder(t *testing.T) {
	...
	if claimed != pagevisitclaim.Taken {
...
func TestDeferringAURLNoHolderClaimedFails(t *testing.T) {
	if _, err := pageVisitClaims(t).Defer(context.Background(), orderID, pageURL(t)); err == nil {
