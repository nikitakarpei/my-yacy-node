...
import (
	...
	"github.com/nikitakarpei/yacy-rwi-node/yacycrawler/internal/pendingpagevisits/jetstream"
)
...
func pendingPageVisits(t *testing.T, js natsjetstream.JetStream) uint64 {
	t.Helper()
	stream, err := js.Stream(context.Background(), pendingpagevisit.StreamName)
	if err != nil {
		t.Fatal(err)
	}
...
func TestPublishPutsThePendingVisitOnTheFrontier(t *testing.T) {
	...
	stream, err := js.Stream(ctx, pendingpagevisit.StreamName)
	if err != nil {
		t.Fatal(err)
	}
	...
	read, err := pendingpagevisit.UnmarshalPendingPageVisit(msg.Data())
	if err != nil {
...
func TestPublishTheSameURLTwiceLeavesOneMessage(t *testing.T) {
	...
	if pending := pendingPageVisits(t, js); pending != 1 {
...
func TestPublishTheSameURLForAnotherOrderKeepsBoth(t *testing.T) {
	...
	for _, orderID := range []string{"o1", "o2"} {
		if err := publisher.Publish(ctx, pendingpagevisit.PendingPageVisit{
	...
	if pending := pendingPageVisits(t, js); pending != 2 {
