...
holds only the peers that explicitly signed up with it keeps most of the existing realm out.
...
It confirms a peer the way any YaCy peer does before it holds it, keeps confirming, and drops a
peer that stops answering. The hello answers of its peer in a realm list the held peers of the
