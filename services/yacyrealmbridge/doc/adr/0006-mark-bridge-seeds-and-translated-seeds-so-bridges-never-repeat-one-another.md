...
Every translated seed carries a translation mark in its tags, signed with the bridge's key over
the seed it marks. A bridge reads only translation marks signed by keys its operator trusts; any
other translation mark is no mark. A bridge does not translate a trusted translation onward, and
does not translate a native peer whose hash a trusted translation in the receiving realm already
carries.
...
in the realm it serves, so a further realm can be bridged onto one that is bridged already. Stock YaCy stores a seed as one
...
anyway. A seed under an untrusted translation mark reads as a native peer, so an untrusted bridge's translations can
