...
availability. A pool allocation is one atomic create in a bucket, so two instances never hand
out one port. Operators run NATS with JetStream next to the bridge. The bridge's addresses in a
