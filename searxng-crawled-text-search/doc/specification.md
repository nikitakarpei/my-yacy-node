...
document carrying a title, canonical URL, and text. SearXNG's built-in `elasticsearch` engine
can query that index, but it always renders hits as a generic key/value table with no `url`
field — a shape `searxng-result-router` cannot rewrite, since it only rewrites the `url` field
of a standard result. `searxng-crawled-text-search` is a SearXNG engine module that queries the
same Elasticsearch index and returns each hit as a standard result carrying its title, URL, and
matched text, so it composes with `searxng-result-router` and closes the loop from crawl to
...
* For each Elasticsearch hit, the engine SHALL return a standard SearXNG result carrying the
  document's title, canonical URL, and matched text.
...
* Relevance ordering is whatever Elasticsearch's full-text match returns; the engine applies no
  ranking of its own.
* The engine reads the document shape `yacytextindexer` writes; changing that shape requires
  changing this engine too, not just its configuration.
